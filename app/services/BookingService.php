...
class BookingService
{
    ...
    public function calculateRooms(array $guestData)
    {
        $dp = [];
        $allocation = [];
        $roomAllocation = [];
            ...
            for ($c = 0; $c <= $childrenUnder13; $c++) {
                ...
                if ($a > 0) {
                    ...
                    if ($cost < $dp[$a][$c]) {
                        ...
                        $roomAllocation[$a][$c] = array_merge($roomAllocation[$a - 1][$c], [['type' => 'SINGLE_ROOM', 'guest' => $guestData[$a - 1]]]);
                    }
                }
                ...
                if ($a > 1 && $c > 0) {
                    ...
                    if ($cost < $dp[$a][$c]) {
                        ...
                        $roomAllocation[$a][$c] = array_merge($roomAllocation[$a - 2][$c - 1], [['type' => 'DOUBLE_ROOM', 'guests' => [$guestData[$a - 2], $guestData[$a - 1], $guestData[$c - 1]], 'extra' => $guestData[$c - 1]]]);
                    }
                }
                ...
                if ($a > 0 && $c > 0) {
                    ...
                    if ($cost < $dp[$a][$c]) {
                        ...
                        $roomAllocation[$a][$c] = array_merge($roomAllocation[$a - 1][$c - 1], [['type' => 'SINGLE_ROOM', 'guest' => $guestData[$a - 1], 'extra' => $guestData[$c - 1]]]);
                    }
                }
                ...
                if ($a > 2 && $c > 0) {
                    ...
                    if ($cost < $dp[$a][$c]) {
                        ...
                        $roomAllocation[$a][$c] = array_merge($roomAllocation[$a - 3][$c - 1], [['type' => 'TRIPLE_ROOM', 'guests' => [$guestData[$a - 3], $guestData[$a - 2], $guestData[$a - 1], $guestData[$c - 1]], 'extra' => $guestData[$c - 1]]]);
                    }
                }
                // Allocate double room for children only
                if ($c == 1) {
                    $cost = $dp[$a][$c - 1] + self::SINGLE_ROOM_RATE;
                    if ($cost < $dp[$a][$c]) {
                        $dp[$a][$c] = $cost;
                        $allocation[$a][$c] = [$allocation[$a][$c - 2][0], $allocation[$a][$c - 2][1] + 1, $allocation[$a][$c - 2][2], $allocation[$a][$c - 2][3]];
                        $roomAllocation[$a][$c] = array_merge($roomAllocation[$a][$c - 2], [['type' => 'SINGLE_ROOM', 'guests' => [$guestData[$c - 2], $guestData[$c - 1]]]]);
                    }
                }
                // Allocate double room for children only
                if ($c > 1) {
